...
if __name__ == "__main__":
    ### use oates control variates 
    ## experiments with polynomial covariates
    start_time = time.time()
    vary_dims = [2, 5, 10, 50]         # varying dimensions
    for dim in vary_dims:              # loop over dimensions
        ...
        N_epochs = [10, 20, 30, 40, 50] # number of epochs
        num_epochs = len(N_epochs)
        mc_err, cv_err, full_err, comp_time = np.zeros((nrep, num_epochs)),np.zeros((nrep, num_epochs)),\
            np.zeros((nrep, num_epochs)),np.zeros((nrep, num_epochs))
        ...
        for i in range(nrep):
            ...
            lr, gamma = 2.e-3, .5
            for ind,epoch in enumerate(N_epochs):
                cv1 = SteinSecondOrderQuadPolyCV(D_in, sigma)       # linear control variate
                res = cv1.train(X, Y, integral,lr=lr, gamma=gamma, clip_value=200.0, train_perc=train_perc,\
                    valid_perc=valid_perc, N_epochs=epoch,  weight_decay= 1.0, batch_size = 100)
                mc_err[i,ind], cv_err[i,ind], full_err[i, ind], comp_time[i,ind] = res[:4]
            ## use classical method
            res1 =cv1.classical_way(X, Y, integral, train_perc=train_perc)
            cls_err[i], cls_full[i], cls_time[i] = res1[1:4]
        ## time takes to run the experiment
        mn_cv_err = np.abs(cv_err).mean(axis=0)
        mn_mc_err = np.abs(mc_err).mean(axis=0)
        mn_cls_err = np.abs(cls_err).mean()
        mn_comp_time = np.abs(comp_time).mean(axis=0)
        mn_cls_time = np.abs(cls_time).mean() 
        ## save the results into a pickle file
        filename = 'dim'+str(dim)+'train'+str(train_perc)+'test'+str(valid_perc)
        ## pickle dump objects into a file
        with open(filename, 'wb') as f:
            pickle.dump([mn_cv_err,mn_mc_err,mn_cls_err,mn_cls_time,mn_comp_time], f)   
